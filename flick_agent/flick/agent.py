...
from .tools import book_football_field, list_field_availabilities
...
class FlickAgent:
    ...
    async def create_agent(self):
        ...
        self.agent = Agent(
            ...
            name="flick_agent",
            description="Helps coordinate football games with friends",
...
async def setup():
    ...
    host = FlickAgent(remote_agent_urls=friend_urls)

    # Step 2: Actually create the AI agent (this does async setup under the hood)
    agent = await host.create_agent()
    print("✅ Host Agent is ready to coordinate football games!")
    return agent


# For ADK web interface, create the agent instance at module level
# nest_asyncio is already applied at the top of the file
