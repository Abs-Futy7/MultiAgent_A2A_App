...
# In-memory database for field schedules, mapping date to a dictionary of time slots and party names
FIELD_SCHEDULE: Dict[str, Dict[str, str]] = {}
...
generate_field_schedule()


def list_field_availabilities(date: str) -> dict:
    """
    List available and booked time slots for a given date.
    """
    if date not in FIELD_SCHEDULE:
        return {
            "status": "error",
    ...
    daily_schedule = FIELD_SCHEDULE[date]
...
def book_football_field(date: str, start_time: str, end_time: str, reservation_name: str) -> dict:
    """
    ...
    if start_time not in FIELD_SCHEDULE[date]:
        return {"status": "error", "message": f"Invalid time. Try 08:00, 09:00, or 10:00."}

    if FIELD_SCHEDULE[date][start_time] != "unknown":
        return {
            "status": "error",
            "message": f"Slot {start_time} on {date} already booked by {FIELD_SCHEDULE[date][start_time]}.",
        }

    FIELD_SCHEDULE[date][start_time] = reservation_name
